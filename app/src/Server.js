...
const { joinMeeting, makeURLByEmail } = require('./controller/meeting.controller.js');
...
const views = {
    ...
    newRoom: path.join(__dirname, '..', 'src/success_templates/thank_you.html'),
...
function startServer() {
    ...
    app.use('/qprofile',meetingRoutes)
    // Logs requests
    ...
    app.get('/join/', async (req, res) => {
        if (Object.keys(req.query).length > 0) {
            ...
            if (token) {
                ...
                const allowRoomAccess = isAllowedRoomAccess('/join/params', req,res, hostCfg, authHost, roomList, room);
    ...
    app.get('/join/:roomId', async(req, res) => {
        if (req.query && (req?.query?.reviewerid || req?.query?.candidateid || req?.query?.corporateid)) {
            const allowRoomAccess = await isAllowedRoomAccess(
                '/join/:roomId',
                req,
                res,
                hostCfg,
            ...
            console.log("allowRoomAccess ",allowRoomAccess)
            if (allowRoomAccess) {
    ...
    app.get('/email', async (req, res) => {
        ...
        const meetingid = req.query.roomId
        ...
        if (email && meetingid) {
          ...
          let result = await makeURLByEmail(req, res, email, meetingid);
          console.log('22 route ',result)
          if (result && result?.success && result?.success?.length > 0) {
            try {
              const meetingId = result.success[0].roomId;
              ...
              console.log('result 21 route', `/join/${meetingId}?${queryMsg}=${tokenId}`)
              const url = `/join/${meetingId}?${queryMsg}=${tokenId}`;
    ...
    async function isAllowedRoomAccess(logMessage, req,res,hostCfg, authHost, roomList, roomId) {
        let allowRoomAccess;
        if (roomId && req) {
            let result = await joinMeeting(req, res);
            ...
            if(result == "success"){
                ...
                res.sendFile(path.join(__dirname, '..', 'src/error_templates/unauthorized.html'))    
            }else if(result == "meeting_invalid"){
