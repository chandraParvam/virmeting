 ...
 * @link    Official Live demo: https://meeting.dev.qprofiles.com
...
class RoomClient {
    ...
    // async produce(type, deviceId = null, encodings = null, codecOptions = null) {
    //     console.log(`Producing media of type ${type} with deviceId ${deviceId}`);
    
    //     if (!this.sendTransport || this.sendTransport.closed) {
    //         console.error('Transport is closed');
    ...
    //         producer.on('transportclose', () => {
    //             console.log(`Transport closed for producer of type ${type}`);
    //             this.producers.delete(producer.id);
    //         });
    
    //         producer.on('trackended', () => {
    //             console.log(`Track ended for producer of type ${type}`);
    //             this.closeProducer(type);
    //         });
    ...
    closeProducer(type) {
        // Log the current state of producer labels and producers
        console.log('Current producer labels:', this.producerLabel);
        console.log('Current producers:', this.producers);
    
        // Check if the producer label exists
        if (!this?.producerLabel?.has(type)) {
        ...
        // Check if the producer exists in this.producers before trying to close it
        if (this.producers.has(producer_id)) {
            this.producers.get(producer_id).close();
            this.producers.delete(producer_id);
        } else {
            console.log('There is no producer with the ID ' + producer_id);
        }
                ...
                if (result.isConfirmed) {
                    ...
                    startScreenButton.onclick = async () => {
                        try {
                            ...
                            handleMediaError(error, mediaType.screen);
                        }
                    };
...
function handleMediaError(error, type) {
    console.error('Access denied for ' + type + ' device [' + error.name + ']: ' + error.message);
    alert('Error accessing ' + type + ' device: ' + error.message + '\nCheck the common getUserMedia errors: https://blog.addpipe.com/common-getusermedia-errors/');
}
